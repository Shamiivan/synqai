...
export function createRouter(dependencies: RouterDependencies) {
  return {
    /** Entry point: runs through BAML router to decide which agent to hand off to. */
    route: (thread: Thread) => runRouter(thread, dependencies),
    /** Resume: dispatch directly to a known agent, bypassing the BAML router. */
...
async function runRouter(thread: Thread, deps: RouterDependencies): Promise<AgentResult> {
  ...
  switch (nextStep.intent) {
    ...
    case "handoff": {
      ...
      // so the agent sees a clear instruction even on follow-ups
      ...
      const result = await runner(thread, log.child(agentName));
...
/** Direct dispatch to a specific agent — used when resuming a paused run. */
async function routeToAgent(agent: string, thread: Thread, deps: RouterDependencies): Promise<AgentResult> {
  ...
  const result = await runner(thread, log.child(agent));
  const intent = getLastIntent(result);
