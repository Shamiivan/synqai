...
/**
 * Classifies a Google Calendar API error into a structured category.
 * The LLM uses `code` and `reason` to decide whether to retry, ask the user, or give up.
 */
export function classifyCalendarError(err: unknown): ClassifiedError {
  const e = err as any;
