...
import type { SheetsAgentDependencies, SheetsTools, Logger } from "@synqai/contracts";
...
export function createSheetsAgent(dependencies: SheetsAgentDependencies) {
  return {
...
async function agentLoop(thread: Thread, deps: SheetsAgentDependencies): Promise<Thread> {
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    const nextStep = await baml.sheetsNextStep(serialized, TODAY) as any;
    ...
    try {
      switch (nextStep.intent) {
        case "create_spreadsheet":
          result = await tools.handleCreateSpreadsheet(nextStep);
          break;
        case "get_spreadsheet":
          result = await tools.handleGetSpreadsheet(nextStep);
          break;
        case "read_values":
          result = await tools.handleReadValues(nextStep);
          break;
        case "write_values":
          result = await tools.handleWriteValues(nextStep);
          break;
        case "append_rows":
          result = await tools.handleAppendRows(nextStep);
          break;
        case "clear_range":
          result = await tools.handleClearRange(nextStep);
          break;
        case "add_sheet":
          result = await tools.handleAddSheet(nextStep);
          break;
        case "list_spreadsheets":
          result = await tools.handleListSpreadsheets(nextStep);
          break;
        default:
