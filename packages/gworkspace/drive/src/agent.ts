...
import type { DriveAgentDependencies, DriveTools, Logger } from "@synqai/contracts";
...
export function createDriveAgent(dependencies: DriveAgentDependencies) {
  return {
...
async function agentLoop(thread: Thread, deps: DriveAgentDependencies): Promise<Thread> {
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    let nextStep: any;
    try {
      nextStep = await baml.driveNextStep(serialized, TODAY) as any;
    } catch (err: any) {
    ...
    try {
      switch (nextStep.intent) {
        case "search_files":
          result = await tools.handleSearchFiles(nextStep);
          break;
        case "get_file":
          result = await tools.handleGetFile(nextStep);
          break;
        case "create_folder":
          result = await tools.handleCreateFolder(nextStep);
          break;
        case "move_file":
          result = await tools.handleMoveFile(nextStep);
          break;
        case "copy_file":
          result = await tools.handleCopyFile(nextStep);
          break;
        case "rename_file":
          result = await tools.handleRenameFile(nextStep);
          break;
        case "trash_file":
          result = await tools.handleTrashFile(nextStep);
          break;
        case "share_file":
          result = await tools.handleShareFile(nextStep);
          break;
        case "list_permissions":
          result = await tools.handleListPermissions(nextStep);
          break;
        default:
