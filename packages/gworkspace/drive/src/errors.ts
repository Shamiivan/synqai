...
export function classifyDriveError(err: unknown): ClassifiedError {
  const e = err as any;
  ...
  if (status === 401 || status === 403) {
    ...
    if (reason === "storageQuotaExceeded") {
      return { code: "storage_full", reason, message: "Google Drive storage quota exceeded.", retryable: false };
    }
