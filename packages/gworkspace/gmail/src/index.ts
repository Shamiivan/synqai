...
import { createGmailAgent, getLastIntent, getLastMessage } from "./agent";
import { getGmailClient, userId } from "./google-auth";
...
async function main() {
  ...
  if (!input) {
    console.error(
      'Usage: npx tsx src/index.ts "check my email"',
    );
    process.exit(1);
  }

  const tools = createGmailTools({ gmail: getGmailClient(), userId });
  const agent = createGmailAgent({
    baml: { gmailNextStep: (thread, today) => b.GmailNextStep(thread, today) },
    tools,
    log: noopLogger,
  });
